    ...
    render() {
        return (
            <>
                ...
                <h4>
                    {web3.utils.fromWei(this.props.sentTokenQty)} {this.props.sentTokenSymbol}
                    {this.props.status === 'FULFILLED' &&
                        <span>&nbsp;(${ Number(web3.utils.fromWei(this.props.sentTokenQty)) * _.mean(this.props.sentTokenOnDollarPrice ?? [])})</span>
                    }
                </h4>
                {/* <p>Feb 2. 2019, 9:21am PST</p> */}
                <p>{moment.unix(this.props.sentOnTime).format("MMM D[. ]YYYY[, ]h[:]mma zz")}</p>
