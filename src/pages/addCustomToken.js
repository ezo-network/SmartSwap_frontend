...
export default class AddCustomToken extends PureComponent {
  ...
  componentDidMount() {
    const networkConfig = this.getNetworkConfig(this.state.selectedSourceTokenData.chainId);
  ...
  getTokenInfo = async () => {
    try {
      if(web3Js.utils.isAddress(this.state.selectedSourceTokenData.address)){
        ...
        const erc20TokenContract = new ERC20TokenContract(this.props.web3Instance, this.state.selectedSourceTokenData.address);
